...
# Dataset names
datasets <- c("K562","E7.5_rep1","E7.5_rep2","E8.5_rep1","E8.5_rep2")
...
dir_create("method_wise_metrics_10kEdges_Combined_GT")
outdir <- "method_wise_metrics_10kEdges_Combined_GT"
...
for (metric in c("Precision", "Recall", "F1-score")) {
  ...
  labs(
    title = paste0(metric, " Comparison Across Methods (Sorted by K562 ", metric, ")"),
    x = "Dataset", y = metric
  ) +
  ...
  ggsave(paste0(outdir,"/",metric, "_comparison_sorted_by_K562_", metric, ".pdf"),
...
for (metric in c("Precision", "Recall", "F1-score")) {
  ...
  y_limit <- max(0.01, round(max_score * 1.05, 3))
