...
reproduce_ROC_PR_plots_full_space <- function(input_dir, 
                                              output_dir, 
  ...
  create_pr_gap_plot <- function(combined_pr, results, method_colors, output_path,
    ...
    for (i in 2:nrow(ordered_df)) {
      ...
      lines(x = df_i_subsampled$recall, y = df_i_subsampled$precision,
            type = "l", col = ordered_df$col[i], lwd = 2, lty = this_lty)
    }
  ...
  evaluate_dataset <- function(dataset_name, dataset_input_dir, ground_truth_file, dataset_output_dir) {
    ...
    cat("Tested TFs:", length(tested_tfs), " | Tested targets:", length(tested_targets), "\n")
    
    # --- Build the COMPLETE TF-target universe (used for AUPRC only) -----
    # Every possible TF-target combination within the tested space.
    # Edges not recovered by a method receive score = 0.
    # This ensures the random baseline and real AUPRC are computed over
    # the full evaluation space, addressing reviewer concern about inflated
    # random AUPRC due to missing unrecovered edges.
    complete_universe <- expand.grid(
      tf     = tested_tfs,
    ...
    cat("Complete TF-target universe size:", nrow(complete_universe), "\n")
    cat("Positives in universe:", sum(complete_universe$label == 1),
        " | Negatives:", sum(complete_universe$label == 0), "\n")
    ...
    for (i in 1:nrow(method_info)) {
      ...
      cat("  Total edges inferred:", n_total_edges, "\n")
      
      # Optionally filter to the tested TF-target space
      ...
      # Add labels to inferred edges (used for AUROC)
      inferred <- inferred %>%
        mutate(
          pair  = paste(tf, target, sep = "_"),
          label = ifelse(pair %in% gt_pairs1, 1, 0)
      ...
      # ----------------------------------------------------------------
      # AUROC — computed on INFERRED edges only (original approach)
      # Kept as-is per reviewer scope: only AUPRC is being corrected
      # ----------------------------------------------------------------
      positives_auroc <- inferred %>% filter(label == 1)
      negatives_auroc <- inferred %>% filter(label == 0)
      
      if (nrow(positives_auroc) == 0 || nrow(negatives_auroc) == 0) {
        cat("  Insufficient positive or negative examples for", method_name, ". Skipping.\n")
        next
      }
      
      cat("  AUROC — Positives:", nrow(positives_auroc), "| Negatives:", nrow(negatives_auroc), "\n")
      
      # Balanced 1:1 sampling for AUROC
      if (nrow(negatives_auroc) < nrow(positives_auroc)) {
        pos_sampled <- positives_auroc %>% sample_n(nrow(negatives_auroc))
        neg_sampled <- negatives_auroc
      } else {
        pos_sampled <- positives_auroc
        neg_sampled <- negatives_auroc %>% sample_n(nrow(positives_auroc))
      }
      
      auroc_data  <- bind_rows(pos_sampled, neg_sampled)
      roc_obj     <- roc(response  = auroc_data$label,
      ...
      # ----------------------------------------------------------------
      # AUPRC — computed on COMPLETE universe (reviewer fix)
      # Merge inferred scores into the full TF-target space.
      # Unrecovered edges receive score = 0, so the method is penalised
      # for edges it failed to predict, and the positive rate correctly
      # reflects the full evaluation space.
      # ----------------------------------------------------------------
      full_inferred <- complete_universe %>%
        left_join(inferred %>% select(tf, target, score),
                  by = c("tf", "target")) %>%
        mutate(score = ifelse(is.na(score), 0, score))
      
      cat("  AUPRC universe — Total pairs:", nrow(full_inferred),
          "| Recovered:", sum(full_inferred$score > 0), "\n")
      
      positives_auprc <- full_inferred %>% filter(label == 1)
      negatives_auprc <- full_inferred %>% filter(label == 0)
      
      cat("  AUPRC — Positives:", nrow(positives_auprc),
          "| Negatives:", nrow(negatives_auprc), "\n")
      
      # 1:10 sampling for AUPRC from the complete universe
      neg_sampled_10x <- negatives_auprc %>%
        sample_n(size = min(nrow(positives_auprc) * 10, nrow(negatives_auprc)))
      
      pr_obj      <- pr.curve(scores.class0 = positives_auprc$score,
      ...
      # ----------------------------------------------------------------
      # Random AUPRC — shuffle scores across the COMPLETE universe
      # (reviewer fix)
      # Scores are randomised over all TF-target pairs including
      # unrecovered edges (score = 0 before shuffling), so the random
      # baseline truly reflects the positive rate in the full space
      # rather than the inflated enriched set of recovered edges only.
      # ----------------------------------------------------------------
      full_inferred_random <- full_inferred %>%
        mutate(score = runif(n()))
      
      positives_r       <- full_inferred_random %>% filter(label == 1)
      negatives_r       <- full_inferred_random %>% filter(label == 0)
      neg_sampled_10x_r <- negatives_r %>%
        sample_n(size = min(nrow(positives_r) * 10, nrow(negatives_r)))
      
      pr_obj_random <- pr.curve(scores.class0 = positives_r$score,
      ...
      results <- rbind(results, data.frame(
        ...
        n_positives       = nrow(positives_auroc),
        n_negatives       = nrow(negatives_auroc),
        stringsAsFactors  = FALSE
      ))
      
      cat("  AUROC:", round(auroc_value, 4),
          "| AUPRC:", round(auprc_value, 4),
          "| Random AUPRC:", round(auprc_random, 4), "\n")
    }
