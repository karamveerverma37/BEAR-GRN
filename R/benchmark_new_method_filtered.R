...
benchmark_new_method <- function(dataset_name,
  ...
  # Add new method to method info (placed first so it appears first in plots)
  ...
  # ---------------------------------------------------------------------------
  # Build the COMPLETE TF-target universe (used for AUPRC and random AUPRC)
  # Every possible TF-target combination within the tested space.
  # Edges not recovered by a method receive score = 0, ensuring the random
  # baseline and real AUPRC reflect the true positive rate in the full space.
  # ---------------------------------------------------------------------------
  ...
  # Ensure new method appears first in results and plots
  ...
  # Color scheme — red for new method, Dark2 palette for existing methods
  unique_methods <- unique(results$method)
