...
#'
#' The random AUPRC baseline is computed over the FULL tested TF-target space
#' (all combinations of tested TFs and tested targets from the ground truth),
#' with unrecovered edges assigned a score of 0 and random scores drawn from
#' Uniform(0,1). This ensures the baseline reflects the true background positive
#' rate and is consistent across methods for a given dataset.
#'
...
#'   group_by summarise sample_n n
#' @importFrom tidyr expand_grid
#' @importFrom pROC roc auc
#' @importFrom PRROC pr.curve
#' @importFrom ggplot2 ggplot aes geom_line geom_abline labs theme_classic theme
#'   element_blank element_line element_text scale_color_manual coord_fixed ggsave
#' @importFrom tibble tibble
#' @importFrom RColorBrewer brewer.pal
#' @importFrom grDevices pdf dev.off
#' @importFrom graphics par lines legend plot
...
reproduce_ROC_PR_plots1 <- function(input_dir,
                                   output_dir,
  ...
  # ---------------------------------------------------------------------------
  # Helper: subsample large data frames for plotting
  # ---------------------------------------------------------------------------
  subsample_data <- function(df, max_points = 10000) {
    int.length <- nrow(df)
    if (int.length <= max_points) return(df)

    vec.subsamp_index <- sample(1:int.length, max_points)
    vec.subsamp_index.padded <- c(
      seq(1, min(10000, int.length), 10),
      vec.subsamp_index,
  ...
  # ---------------------------------------------------------------------------
  # Helper: PR gap plot using base graphics
  # ---------------------------------------------------------------------------
  create_pr_gap_plot <- function(combined_pr, results, method_colors, output_path,
    ...
    for (i in 2:nrow(ordered_df)) {
      ...
      lines(
        x    = df_i_subsampled$recall,
        y    = df_i_subsampled$precision,
        type = "l",
  ...
  # ---------------------------------------------------------------------------
  # Main evaluation function for a single dataset
  # ---------------------------------------------------------------------------
  evaluate_dataset <- function(dataset_name, dataset_input_dir,
                               ground_truth_file, dataset_output_dir) {
    ...
    gt_pairs1      <- unique(paste(ground_truth$Source, ground_truth$Target, sep = "_"))
    ...
    cat("Tested TFs:", length(tested_tfs),
        "| Tested targets:", length(tested_targets), "\n")

    # ---- Build the full tested TF-target space (used for random baseline) ---
    # Every combination of tested TFs x tested targets that exists in the
    # ground truth evaluation universe. Edges absent from a method's output
    # will be assigned score = 0 here, matching the implicit assumption that
    # un-predicted edges have the lowest possible confidence.
    full_space <- expand.grid(
      tf     = tested_tfs,
      target = tested_targets,
      stringsAsFactors = FALSE
    ) %>%
      mutate(
        pair  = paste(tf, target, sep = "_"),
        label = ifelse(pair %in% gt_pairs1, 1, 0)
      )

    cat("Full tested space:", nrow(full_space), "edges",
        "| Positives:", sum(full_space$label),
        "| Background positive rate:", round(mean(full_space$label), 5), "\n")
    ...
    results <- data.frame(
      ...
      n_total_edges    = integer(),
      n_evaluable_edges = integer(),
      n_positives      = integer(),
      n_negatives      = integer(),
      stringsAsFactors = FALSE
    )
    ...
    for (i in 1:nrow(method_info)) {
      ...
      if (use_filtered_approach) {
        inferred <- inferred %>%
          filter(tf %in% tested_tfs & target %in% tested_targets)
        cat("  Filtered to tested space:", nrow(inferred), "edges\n")
      }

      # Add labels to predicted edges
      inferred <- inferred %>%
        mutate(
          pair  = paste(tf, target, sep = "_"),
          label = ifelse(pair %in% gt_pairs1, 1, 0)
      ...
      cat("  Positives:", nrow(positives),
          "| Negatives:", nrow(negatives),
          "| Total predicted (in tested space):", nrow(inferred), "\n")

      # ---- AUROC --------------------------------------------------------------
      # Balance classes for AUROC (standard practice)
      ...
      roc_obj    <- roc(
        response  = auroc_data$label,
        predictor = auroc_data$score,
      ...
      # ---- AUPRC (real scores) -----------------------------------------------
      # 10:1 negative oversampling for AUPRC
      neg_sampled_10x <- negatives %>%
        sample_n(size = min(nrow(positives) * 10, nrow(negatives)))
      ...
      # ---- AUPRC (random baseline over FULL tested space) --------------------
      #
      # KEY FIX 
      #
      # Previously the random baseline was computed only on the edges the method
      # predicted, which is a subset of the full tested space that is already
      # enriched for true positives (because methods tend to output high-
      # confidence, likely-true interactions). Shuffling scores within that
      # enriched set gave an inflated random AUPRC that did not reflect the
      # true background positive rate.
      #
      # Correct approach:
      #   1. Start from the full tested TF-target space (all combinations of
      #      tested TFs x tested targets as defined by the ground truth).
      #   2. Merge in the method's actual scores; edges the method did NOT
      #      predict receive score = 0 (lowest possible confidence).
      #   3. Draw independent Uniform(0,1) random scores for every edge in
      #      this full space — this is the random classifier.
      #   4. Compute AUPRC on the full space with 10:1 negative oversampling.
      #
      # This ensures:
      #   - The random baseline uses the same evaluation universe as the ground
      #     truth benchmark (fair comparison).
      #   - The expected random AUPRC equals the true background positive rate
      #     (n_positives / total_space), consistent across all methods for a
      #     given dataset.
      #   - Un-recovered edges (score = 0) are correctly treated as the lowest-
      #     confidence predictions rather than being silently excluded.

      full_space_random <- full_space %>%
        mutate(score = runif(n()))   # independent random scores over full space

      positives_r    <- full_space_random %>% filter(label == 1)
      negatives_r    <- full_space_random %>% filter(label == 0)
      neg_sampled_10x_r <- negatives_r %>%
        sample_n(size = min(nrow(positives_r) * 10, nrow(negatives_r)))
      ...
      cat("  AUROC:", round(auroc_value, 4),
          "| AUPRC:", round(auprc_value, 4),
          "| Random AUPRC (full space):", round(auprc_random, 4), "\n")
      ...
      results <- rbind(results, data.frame(
        ...
        n_evaluable_edges = nrow(inferred),
    ...
    results <- results %>%
      mutate(
        method_auroc = paste0(method, " (", round(AUROC, 3), ")"),
    ...
    random_pr_combined <- random_pr_combined %>%
      mutate(
        curve_type   = "Randomized",
        legend_label = paste0(method, " Random AUPRC=", round(AUPRC_random, 3))
      )

    pr_combined <- pr_combined %>%
      mutate(
        curve_type   = "Normal",
        legend_label = paste0(method, " AUPRC=", round(AUPRC, 3))
      )
    ...
    # ---- Save plots ---------------------------------------------------------
    ggsave(file.path(dataset_output_dir, "ROC_plot.png"),
           plot = roc_plot, width = 8, height = 6, dpi = 300)
    ggsave(file.path(dataset_output_dir, "ROC_plot.pdf"),
           plot = roc_plot, width = 8, height = 6, device = "pdf")
    ggsave(file.path(dataset_output_dir, "ROC_plot.eps"),
           plot = roc_plot, width = 8, height = 6, device = "eps")
  ...
  for (dataset_name in dataset_dirs) {
    ...
    tryCatch({
      result <- evaluate_dataset(
        dataset_name, dataset_input_dir, ground_truth_file, dataset_output_dir
      )
      if (!is.null(result)) {
