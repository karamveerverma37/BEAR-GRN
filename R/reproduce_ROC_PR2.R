...
reproduce_ROC_PR_plots_full_space <- function(input_dir, 
  ...
  evaluate_dataset <- function(dataset_name, dataset_input_dir, ground_truth_file, dataset_output_dir) {
    ...
    # --- Build the COMPLETE TF-target universe ----------------------------
    # Every possible TF-target combination within the tested space.
    # Edges not recovered by a method will receive score = 0.
    ...
    for (i in 1:nrow(method_info)) {
      ...
      # --- Merge with COMPLETE universe ----------------------------------
      # Left-join so every TF-target pair is present; unrecovered edges get score = 0
      full_inferred <- complete_universe %>%
        left_join(inferred, by = c("tf", "target")) %>%
        mutate(score = ifelse(is.na(score), 0, score))
      
      n_evaluable_edges <- sum(!is.na(full_inferred$score))  # always all rows now
      cat("  Edges after merging with complete universe:", nrow(full_inferred), "\n")
      cat("  Edges recovered by method:", sum(full_inferred$score > 0), "\n")
      
      # Split into positives and negatives
      positives <- full_inferred %>% filter(label == 1)
      negatives <- full_inferred %>% filter(label == 0)
      ...
      # ----------------------------------------------------------------
      # AUROC — balanced 1:1 sampling from the COMPLETE universe
      # ----------------------------------------------------------------
      ...
      # ----------------------------------------------------------------
      # AUPRC — 1:10 sampling from the COMPLETE universe
      # ----------------------------------------------------------------
      neg_sampled_10x <- negatives %>%
        sample_n(size = min(nrow(positives) * 10, nrow(negatives)))
      
      pr_obj      <- pr.curve(scores.class0 = positives$score,
      ...
      # Unrecovered edges (score=0) are included before shuffling,
      # ensuring the random baseline reflects the true positive rate.
      # ----------------------------------------------------------------
      full_inferred_random <- full_inferred %>%
        mutate(score = runif(n()))        # shuffle over ALL pairs, including score-0 edges
      ...
      results <- rbind(results, data.frame(
        ...
        n_evaluable_edges = nrow(full_inferred),
